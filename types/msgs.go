...
// ProposeMsg defines the properties of a proposal for a ballot
...
// ensure DepositMsg implements the sdk.Msg interface
var _ sdk.Msg = (*ProposeMsg)(nil)

// ValidateBasic is called by the SDK automatically.
func (d ProposeMsg) ValidateBasic() sdk.Error {
	if d.Index <= 0 {
		return ErrInvalidParameter("negative index number")
	}
	if d.Candidate <= 0 {
		return ErrInvalidParameter("negative Candidate number")
	}
	if d.Name == "" {
		return ErrInvalidParameter("empty name")
	}
	if err := validateAddress(d.Sender); err != nil {
		return err
	}
...
// Type returns the message type.
// Must be alphanumeric or empty.
func (d ProposeMsg) Type() string {
	return ProposeVoteType
}

// Get some property of the Msg.
func (d ProposeMsg) Get(key interface{}) (value interface{}) {
	return nil
}

// GetSignBytes returns the canonical byte representation of the Msg.
func (d ProposeMsg) GetSignBytes() []byte {
	bz, err := cdc.MarshalBinary(d)
	if err != nil {
		panic(err)
	}
...
// CONTRACT: All signatures must be present to be valid.
// CONTRACT: Returns addrs in some deterministic order.
func (d ProposeMsg) GetSigners() []crypto.Address {
	return []crypto.Address{d.Sender}
}
